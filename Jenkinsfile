pipeline {
    ...
    stages {
        ...
        stage('Scan') {
            steps {
                sh "podman run -v ${HOME}/image.tar:/tmp/image.tar:Z quay.io/sysdig/secure-inline-scan:2 --sysdig-url https://us2.app.sysdig.com --sysdig-token ${SYSDIG_TOKEN} --storage-type docker-archive --storage-path /tmp/image.tar quay.io/sysdig/secure-inline-scan:2 ${params.image_to_scan}"
            }
        }
    }
}
